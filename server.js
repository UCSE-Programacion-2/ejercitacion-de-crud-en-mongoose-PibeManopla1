...
app.put('/equipos/:id', async (req, res) => {
    try {
        ...
        // Validamos manualmente todos los campos requeridos usando el Schema antes de actualizar
        const equipoAValidar = new Equipo(req.body);
        const errorValidacion = equipoAValidar.validateSync();
        
        if (errorValidacion) {
            return res.status(400).json({ error: errorValidacion.message });
        }
